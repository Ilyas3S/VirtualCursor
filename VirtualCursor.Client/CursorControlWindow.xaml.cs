...
{
	public partial class CursorControlWindow : Window
	{
		// ---- UDP и графика ----
		private HolePunchUdpClient _udpClient;
		private ScreenFrameReceiver _frameReceiver;
		...
		private void OnLoaded(object sender, RoutedEventArgs e)
		{
			...
			// Инициализация приёмника кадров
			_frameReceiver = new ScreenFrameReceiver((int)_screenWidth, (int)_screenHeight);
			_frameReceiver.OnFrameReady += OnFrameReady;
			_frameReceiver.OnFrameStats += OnFrameStats;
			...
			//StatusText.Text = "Режим: Клавиатура | Подключено";
		}
		...
		private void OnFrameReady(WriteableBitmap bitmap)
		{
			Dispatcher.Invoke(() =>
			{
				...
				ScreenImage.Width = _screenWidth;
				ScreenImage.Height = _screenHeight;
			});
		}
		...
		private void OnFrameStats(int fps, int blocks)
		{
			Dispatcher.Invoke(() =>
			{
				//StatsText.Text = $"FPS: {fps} | Блоков: {blocks}";
			});
		}

		private void OnUdpDataReceived(IPEndPoint remote, byte[] data)
		{
			// Передаём данные в приёмник кадров
			_frameReceiver?.ProcessData(data);
		}
		...
		private void ToggleMode()
		{
			...
			else
			{
				...
				StatusText.Text = "Режим: Мышь | Подключено";
			}
		}
